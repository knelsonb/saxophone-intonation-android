...
import type { ChannelHandle, MetroOutputRoute, MidiBusState } from './useMidiBusCore';
...
// #26 — the pure, native-free beat-structure / profile-conversion helpers were
// extracted into ./metroHelpers. They are imported here (the React body still
// uses them) AND re-exported so existing `from './useMetronome'` import sites
// (components, screens, the metroScheduling test) keep working unchanged.
import {
  beatsPerBar,
  buildDefaultPattern,
  resizePattern,
  ...
  MAX_NUMERATOR,
  DEFAULT_BEAT_1_MIDI,
  DEFAULT_BEAT_1_VELOCITY,
  DEFAULT_BEAT_N_MIDI,
  DEFAULT_BEAT_N_VELOCITY,
  DEFAULT_SUB_MIDI,
  DEFAULT_SUB_VELOCITY,
} from './metroHelpers';
import type {
  TimeSigPreset,
  TimeSig,
...
// Re-export the pure helpers + their types/constants so the existing public
// surface of './useMetronome' is byte-for-byte unchanged for importers.
export {
  beatsPerBar,
  buildDefaultPattern,
  resizePattern,
...
// #26 — TimeSig / TimeSigPreset / BeatInstrument / Subdivision, the GM drum
// defaults, the BPM / numerator / denominator constants, beatsPerBar, and the
// clamps now live in ./metroHelpers (imported + re-exported above). Only the
// React hook, its state machine, and the MetronomeState interface remain here.
...
// #26 — clampBpm / clampNumerator / clampDrumMidi, buildDefaultPattern,
// resizePattern, validatePatternForBeats, buildInitialProfiles,
// editableToMetroProfile, metroProfileToEditable, applyProfilePatch, and the
// ProfileSlot / PROFILE_COUNT pair were moved verbatim into ./metroHelpers
// (imported + re-exported above). The hook below consumes them unchanged.
